...
SELECT
   ...
   ,CASE
        ...
        WHEN  EXISTS (SELECT 1 FROM UNNEST(download_ambiguous.status)) 
            THEN IF (EXISTS (SELECT 1 FROM UNNEST(download_ambiguous.status) WHERE status IN ("ERROR"))
                        , STRUCT ("credit" AS endpoint, source_bank AS bank) #error_credit_source
                        , NULL) #ignore_main_action
        WHEN 1 > (SELECT SUM(count.with_hash) FROM UNNEST(main_action.status)) #no_hay_hash
            THEN CASE 
                    WHEN EXISTS (SELECT 1 FROM UNNEST(main_action.status) WHERE status IN ("ERROR")) 
                        THEN STRUCT ("action" AS endpoint, source_bank AS bank) #error_action
                    WHEN EXISTS (SELECT 1 FROM UNNEST(main_action.status) WHERE status IN ("REJECTED")) AND  download_source.count IS NULL
