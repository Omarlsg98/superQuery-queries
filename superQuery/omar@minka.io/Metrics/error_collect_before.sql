...
UNION ALL
SELECT
    "sep-apr" AS category ,COUNT(transfer_id) AS number,  CAST("2020-07-01" AS TIMESTAMP) AS measured_on
FROM
    minka-ach-dw.ach_tin_before.transfer_20200701
WHERE
    status NOT IN ("REJECTED", "COMPLETED")
...
UNION ALL
SELECT
    "jul-now" AS category ,COUNT(transfer_id) AS number, CAST("2020-07-01" AS TIMESTAMP) AS measured_on
FROM
    minka-ach-dw.ach_tin_before.transfer_20200701
WHERE
    status NOT IN ("REJECTED", "COMPLETED")
...
UNION ALL
SELECT
    "all" AS category  ,COUNT(transfer_id) AS number,  CAST("2020-07-01" AS TIMESTAMP) AS measured_on
FROM
    minka-ach-dw.ach_tin_before.transfer_20200701
WHERE
    status NOT IN ("REJECTED", "COMPLETED")
